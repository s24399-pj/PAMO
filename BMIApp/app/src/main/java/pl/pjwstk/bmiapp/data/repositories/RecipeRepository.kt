...
class RecipeRepository private constructor() {
    private val allRecipes: List<Recipe> = initializeRecipes()
    ...
    fun getAllRecipes(): List<Recipe> {
        return ArrayList(allRecipes)
    }

    fun getRecipesForDiet(dietType: Int): List<Recipe> {
        return allRecipes.filter { it.dietType == dietType }
    }

    fun getRecipesForDietAndCalories(dietType: Int, calories: Double): List<Recipe> {
        val needLowCalorie = calories < 2000 // Próg kaloryczny

        return allRecipes
            .filter { it.dietType == dietType }
            .filter { it.isLowCalorie == needLowCalorie }
            .take(2) // Zwróć maksymalnie 2 przepisy
    }
